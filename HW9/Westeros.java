...
public class Westeros extends JPanel {
    ...
    public static boolean whitewalkerEnabled = true;
    private Random rand;
    private int wcmchance = 10;// Whitewalkers come out chance in 1000
    private int whitewalkerInitNum = 2;
    private int nightwatcherInitNum = 2;
    public static int whitewalkerNum = 0;

    /**
     * Constructor
     * @param control A ControlPanel instance which is used to determine the next
     *            House to create
     */
    ...
    /**
     * Check to see whether any of the houses can destroy the other houses they
     * are around. Then destroys them.
     */
    public void checkFoodChain() {
        for (House a : houses) {
            for (House other : houses) {
                if (a.collidesWithHouse(other)) {
                    if (a != other && a.canDestroyHouse(other)) {
    ...
    public void checkReproduce() {
        ...
        for (int i = 0; i < currHousesSize; i++) {
            ...
            for (int j = i + 1; j < currHousesSize; j++) {
                ...
                if (houses.get(i).collidesWithHouse(houses.get(j))) {

                    if (houses.get(i).canReproduceWithHouse(houses.get(j))) {

                        House baby = houses.get(i).reproduceWithHouse(
                                houses.get(j));
    ...
    /**
     * Circle of life. Dead houses go back to the soil and Aka remove them from
     * the list. Since Java Garbage collects, once the only reference to the
    ...
    /**
     * This isn't the humane society. This is the Serengeti. Old houses have to
     * die.
     */
    ...
    public void WhitewalkerComeout() {

        for (int i = 0; i < whitewalkerInitNum; i++) {
            houses.add(new Whitewalker(rand.nextInt(WIDTH), rand
                    .nextInt(HEIGHT), bounds));
            whitewalkerNum++;
        }
        for (int i = 0; i < nightwatcherInitNum; i++) {
            houses.add(new Nightwatcher(rand.nextInt(WIDTH), rand
                    .nextInt(HEIGHT / 2), bounds));
        }
    }

    private class TimerListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            if (whitewalkerEnabled && (rand.nextInt(1000) < wcmchance)) {
                // come out
                WhitewalkerComeout();
            }
            moveAll();
